...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        originValue = (1 - self.alpha) * qValue
        newValue = self.alpha * (reward + self.discount * self.getValue(nextState))
        self.q_values[(state, action)] = originValue + newValue
        #util.raiseNotDefined()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        originValue = (1 - self.alpha) * qValue        
        newValue = self.alpha * (reward + self.discount * nextStateValue)
        self.q_values[(state, action)] = originValue + newValue
